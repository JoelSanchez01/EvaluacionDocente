...
export class TableCareerComponent {
  ...
  displayedColumnsAlumnos: string[] = ['clave_c', 'reticula', 'nivel', 'clave', 'nombre', 'nombreA', 'siglas', 'modalidad', 'fecha_termino', 'Botones'];
  ...
  Borrar(id: any) {
    this.otro = Valores.filter(item => item.clave_carrera != id)
    Valores = this.otro;
    this.dataSource = new MatTableDataSource(Valores);
    this.dataSource.paginator = this.paginator;
    this.dataSource.sort = this.sort;
